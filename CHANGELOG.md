...
- Prompt 工程与终端命令安全链路已进一步收口：运行时系统 Prompt、IDE Prompt、`ide_run_command` 工具声明与 Windows MCP 原始命令入口统一强调“只允许短时、非交互、可结束命令”，同时补齐系统目录、注册表、服务、计划任务与磁盘配置等高风险目标的限制。
- IDE 一次性命令执行现在会在“长时间无输出 / 检测到交互式提示 / 超时”时自动停止，并把触发停机的提示行与系统说明一起回传；工具层也改为保留输出头尾，减少长输出下最后报错被截断导致的误判。
- IDE 一次性命令的工作目录校验已收紧到“必须位于当前工作区内”，Windows MCP 原始命令也新增前置安全拦截与更明确的结构化失败信息，避免模型把危险命令发到主进程后才知道被拒绝。
