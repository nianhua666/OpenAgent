...
- Sub2API 本地网关这轮已经从“只认内嵌 exe”升级成“源码工作树优先”：OpenAgent 现在能识别源码目录、源码构建产物和 `git / pnpm / go` 工具链，并允许直接在页面里同步源码与触发构建；当前机器也已经把 Go 安装到 `D:\dev-tools\go`，并在 `D:\AllDocument\Documents\OpenAgent-data\sub2api-runtime\source\sub2api` 成功完成官方前后端构建。当前剩余缺口不再是“源码模式不存在”，而是本地 PostgreSQL / Redis 依赖仍需继续准备与联调，才能把 setup 初始化和完整桌面本地网关工作流跑通。
...
| 2026-03-19 | code | Phase 9 / Sub2API 源码优先接入：`sub2apiRuntime.ts` 现已支持源码目录、源码仓库地址、源码优先运行、源码构建产物识别，以及 `git / pnpm / go` 工具链探测；`Sub2ApiSettings.vue` 新增源码目录、源码同步、源码构建和源码状态展示，不再只围绕内嵌 `sub2api.exe` 工作。 |
| 2026-03-19 | docs | Phase 9 / Sub2API 文档同步：`README.md` 和 `build/sub2api-runtime/README.md` 现已改为“源码工作树优先、内嵌二进制仅作兜底”的说明，避免后续继续把 `build/sub2api-runtime/bin/sub2api.exe` 当作主要维护路径。 |
