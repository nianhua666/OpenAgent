...
- Prompt 工程与命令安全链路已经进一步收口：运行时系统 Prompt、`aiPrompts.ts`、`ide_run_command` 工具声明、IDE 一次性命令工具与 Windows MCP 原始命令入口现在都统一强调“只允许短时、非交互、可结束命令”，并补齐系统目录 / 注册表 / 服务 / 计划任务 / 磁盘配置等高风险目标的硬限制；主进程会在命令长时间无输出、检测到交互式提示或超时后自动停止，并把触发停机的提示行与系统说明一并回传，工具层也改为保留输出头尾，减少“最后的报错被截掉、模型误判卡住原因”的问题。当前剩余工作不再是这条链路缺失，而是继续通过真实 TUI / 长命令人工回归观察误拦截率，并把更细粒度 allowlist / denylist 调整到更稳。
...
| 2026-03-13 | code | Phase 8 Prompt / 终端安全收口：`stores/ai.ts` 与 `aiPrompts.ts` 补齐运行时命令红线，`ai.ts` / `aiTools.ts` 新增并收紧 `ide_run_command` 的工作区边界、非交互约束、输出裁剪与 Windows MCP 原始命令前置拦截，`electron/main.ts` / `electron/mcp.ts` 则补齐交互提示回传、自动停机与更严格的系统级危险命令黑名单，`IDETerminal.vue` 也同步展示一次性命令自动停机策略 |
| 2026-03-13 | review | Phase 8 终端防卡住巡检：确认 AI 一次性命令过去存在“工作目录可被绝对路径绕出工作区、交互提示行可能在自动停机前丢失、长输出易截断最后报错、MCP 原始命令报错过晚”的问题，现已统一改为工作区内校验、先回传触发行再停机、保留输出头尾并在工具层前置安全拦截 |
| 2026-03-13 | test | Phase 8 Prompt / 终端安全验证：`npm.cmd run build` 与 `npm.cmd run smoke:routes` 均通过，确认系统 Prompt、IDE 一次性命令、防卡住守卫与 MCP 命令安全策略接入后未破坏 `/ai`、`/ide`、`/ai-overlay`、`/sub2api` 主链路 |
