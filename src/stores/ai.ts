...
- 若必须执行命令，只能运行短时、非交互、可结束的验证命令；一旦出现长时间无输出、y/n、password、press any key 或菜单选择等迹象，应立即停止并改用更安全的方式
- 不要通过命令删除、覆盖或改写系统目录、系统文件、注册表、服务、计划任务、启动项、磁盘配置，或工作区外的敏感路径
    ...
    if (!capabilities.conversationOnly) {
      sections.push([
        ...
        capabilities.fileControlEnabled && ideWorkspace.value
          ? '- 只有在需要读取 build / test / lint / git 状态等验证结果时，才使用短时、非交互、可结束的工作区命令。'
          : '- 没有工作区文件能力时，只允许执行受控、可验证的只读诊断命令，不要假装能直接修改项目文件。',
        '- 若工具链支持，请优先添加 --yes、--non-interactive、--ci 等参数，避免等待人工输入。',
        '- 不要启动 watch、dev server、REPL、ssh、持续占用终端的后台进程，也不要执行需要密码、y/n、press any key 或菜单选择的命令。',
        '- 不要通过命令删除、覆盖或改写系统目录、系统文件、注册表、服务、计划任务、启动项、磁盘配置，或工作区外敏感路径。',
